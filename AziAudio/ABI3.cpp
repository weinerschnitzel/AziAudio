...
/*
void SETVOL3 () { // Swapped Rate_Left and Vol
	u8 Flags = (u8)(k0 >> 0x10);
	if (Flags & 0x4) { // 288
		if (Flags & 0x2) { // 290
			VolTrg_Left  = *(s16*)&k0;
...
// ********* Make sure these conditions are met... ***********
	if ((AudioInBuffer | AudioOutBuffer | AudioAuxA | AudioAuxC | AudioAuxE | AudioCount) & 0x3) {
		MessageBox (NULL, "Unaligned EnvMixer... please report this to Azimer with the following information: RomTitle, Place in the rom it occurred, and any save state just before the error", PLUGIN_VERSION, MB_OK);
	}
// ------------------------------------------------------------
 	short *inp=(short *)(BufferSpace+0x4F0);
	short *out=(short *)(BufferSpace+0x9D0);
...
/*
void ENVMIXER3 () { // Borrowed from RCP...
	u8  flags = (u8)((k0 >> 16) & 0xff);
	u32 addy = (t9 & 0xffffff);// + SEGMENTS[(t9>>24)&0xf];
	...
	Vol_Right = (k0 & 0xffff); // Needed for future references
...
extern "C" {
	void rsp_run();
	void mp3setup (DWORD k0, DWORD t9, DWORD t8);
}
